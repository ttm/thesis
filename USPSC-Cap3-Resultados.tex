...
\begin{itemize}
	\item Peripherals were found to use more nouns while hubs use more verbs and adverbs.
		The fraction of adjectives did not change as incontestably with respect to connectivity,
		but given that the nouns are more numerous in the periphery, there are more adjectives per noun in the hubs sector texts.
	\item Sentences and messages were found smaller in the more connected sectors although
		punctuation was more incident in the less connected sectors.
	\item The differences found in analyzing Wordnet synset hypernyms were found less well behaved.
		...
		Some of these incidences are more systematic and this analysis assisted by Wordnet is the only semantic analysis we made,
		reasons that motivated our inclusion of these results.
\end{itemize}
Appendix~\ref{ap:textd} presents tables
by explicit counts of greater incidence in each sector throughout the networks.
In the following discussion we display example tables for each set of measurements.
In the measurements derived from synset hypernyms the example tables were less meaningful
because of the greater variability and therefore we present the counts of greater incidence directly.
...
\input{tables/finalSynsetsN_} % uma soh
\FloatBarrier
...
\input{tables/finalSynsetsAS_} % uma soh
\FloatBarrier
...
\input{tables/finalSynsetsV_} % uma soh
\FloatBarrier
...
\input{tables/finalSynsetsR_} % uma soh
\FloatBarrier
\end{itemize}
...
The ending figures of~\cite{textTables} are dedicated to this histogram differences.
