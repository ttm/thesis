...
This open approach to both data and scripts reinforces the scientific aspect of the contribution~\cite{openSci} and mitigates ethical and moral issues involved in researching systems constituted of human individuals~\cite{anPhy,anPhy2}.
...
\item Fraction of morphosyntactic classes, such as adverbs, adjectives, nouns and other POS (Part-Of-Speech) tags.
	We implemented a Brill POS tagger, because of the massive amount of textual data that we had to analyze (the Brill tagger is very fast). We used the ``universal'' tagset described in~\cite{petrov} and trained the tagger with both (full) Brown and (5\% as included in NLTK~\cite{trabNLTK}) Treebank corpus divided in 80\% for training and 20\% for evaluation (in which the tagger achieved $94.95\%$ of accuracy).
