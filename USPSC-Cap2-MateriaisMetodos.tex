...
\item Fraction of morphosyntactic classes, such as adverbs, adjectives, nouns and other POS (Part-Of-Speech) tags.
	We implemented a Brill POS tagger, because of the massive amount of textual data that we had to analyze (the Brill tagger is very fast). We used the ``universal'' tagset described in~\cite{petrov} and trained the tagger with both Brown and Treebank corpus divided in 80\% for training and 20\% for evaluation (in which the tagger achieved $\approx 95\%$ of accuracy).
