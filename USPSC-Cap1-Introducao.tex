...
In the node-edge representation, each node represented as a point while each edge is represented
...
of paradigmatic networks and generic measures for nodes and the network (and edges, but it is less often).

\subsubsection{Basic measures}\label{se:intMea}
...
Next Chapter presents data and methods while the results and discussion are in Chapter~\ref{ch:disc}.
Conclusions and further work are stated in Chapter~\ref{ch:concl}.
Appendixes display auxiliary tables, figures and other results.
