...
and that it is a graph representation of a system found in natural, real or empirical systems.
...
which are here for characterizing basic types
...
\begin{itemize}
	\item The Erdös-Rényi model\footnote{This name is also used for the model in which, for a fixed number of nodes and a fixed number of links, all networks are equally likely. This is the model originally introduces by Paul Erdös and Alfréd Rényi~\cite{erdosOrig}.
	...
	\item Scale-free networks: in which the degree distribution $p(k)$ follows a power law ($p(k)=C.k^{-\alpha}$ where C and k are constants).
...
as is the case with the \emph{complexity theory} in general.
...
\begin{itemize}
	\item criteria for the quality of layouts might include the number of crossing edges or the area of the drawing relative to closest distance between two vertices.
...
\end{align}
	...
	\item Graph drawings are often developed for specific applications e.g. in biology (as for protein and gene interactions), social networks, tree diagrams.
\end{itemize}
...
applied complex networks and text mining criteria for resources recommendation~\cite{participaRecommend} and
...
\section{Further considerations about the present work}
...
This motivated the open software and texts produced, conferences attended, and the endeavors with
...
many fields are deeply related to the subject, which reflected in the number of fields tackled in the thesis
...
Next Chapter presents data, visualization and code infrastructure for using the methods
and obtaining the results in this thesis.
Chapter~\ref{ch:mat} presents the methods while the results and discussion are in Chapter~\ref{ch:disc}.
Conclusions and further work are stated in Chapter~\ref{ch:concl}.
Appendixes display auxiliary tables, figures and resources developed in our research.
