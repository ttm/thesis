...
We choose the definition given online (não seria inline, por ser uma definição dada no corpo principal do texto?), which is closely related to the one given in this footnote, because it is more commonly used nowadays.}: each pair of nodes is connected with a fixed random probability $p$.
